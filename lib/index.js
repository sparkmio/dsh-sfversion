...
export function apply(ctx) {
  ...
  async function visionRequest(dataUrl, mode, model, query, signal) {
    const cfg = config()
    const tokenLimit = mode === 'restore' ? DEFAULTS.restoreMaxTokens : mode === 'ground' ? DEFAULTS.groundMaxTokens : DEFAULTS.describeMaxTokens
    const prompt = mode === 'restore' ? restorePromptFor(query) : mode === 'ground' ? groundPromptFor(query) : promptFor(query)
