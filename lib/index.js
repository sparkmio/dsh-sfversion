...
export function apply(ctx) {
  ...
  async function documentTextBlocks(text, signal, query) {
    ...
    const type = documentType(transfer.name, transfer.mime)
    if (!type || !supportedDocument(transfer.name, transfer.mime)) throw new Error('不支持的文档格式，仅支持 Word/PPT/Excel/PDF/Markdown')
    const doc = await parseDocument({ bytes: transfer.bytes, name: transfer.name, mime: transfer.mime, signal })
    ...
    const context = documentContext(doc, imageResults)
    const around = [transfer.before.trim(), context, transfer.after.trim()].filter(Boolean).join('\n\n')
  ...
  async function resolveToolDocument(document, exec) {
    ...
    if (!type) throw new Error('仅支持 docx/pptx/xlsx/pdf/md/markdown 文档: ' + target.displayPath)
