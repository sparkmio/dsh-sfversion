...
export function apply(ctx) {
  ...
  async function documentTextBlocks(text, signal, query) {
    ...
    if (reference) {
      ...
      const around = [reference.before.trim(), result.text, reference.after.trim()].filter(Boolean).join('\n\n')
  ...
  function contentHasDocumentReference(content) {
    ...
    return content.some((block) => {
      if (block?.type === 'text' && typeof block.text === 'string') return Boolean(parseDocumentReference(block.text) || /\[\[SFV_DOCUMENT_V1\s/i.test(block.text))
  ...
  ctx.on('llm/stream', (options, next) => {
    ...
    return (async function* () {
      const translated = await Promise.all(messages.map(async (message) => ({
        ...message,
        content: await translateDocumentBlocks(message.content, options?.signal, textQuery(message.content)),
      })))
