...
// attachment 发出。新的短引用用 Markdown 文件链接呈现：聊天里只显示文件名，
// 真实字节留在宿主内存，模型调用前再由 visionTranslation 展开。
function documentReferenceMarker(name, token) {
  const label = String(name || 'document').replace(/[\[\]\r\n]/g, '_')
  return `[📎 ${label}](sfv-document://${encodeURIComponent(String(token || ''))})`
}
...
function parseDocumentReference(text) {
  ...
  return { name, token, full: match[0], before: value.slice(0, match.index), after: value.slice(match.index + match[0].length) }
}
...
export function apply(ctx) {
  ...
  async function documentTextBlocks(text, signal, query) {
    ...
    if (reference) {
      const item = resolveDocumentReference(reference.token)
