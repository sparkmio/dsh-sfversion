 ...
 *
 * 配置:DSH Credential `STEPFUN_API_KEY`(dsh credentials set STEPFUN_API_KEY)。
 */
...
export const inject = ['tools', 'credentials', 'fs', 'attachments', 'timer', 'sandboxPolicy']
...
export function apply(ctx) {
  ...
  async function resolveApiKey() {
    ...
    if (resolved === undefined || typeof resolved.value !== 'string' || resolved.value.trim() === '') {
      throw new Error(`未找到 StepFun API Key:请执行 dsh credentials set ${DEFAULTS.credentialRef}`)
    }
  ...
  async function imageTextBlocks(imageBlock, signal) {
    ...
    const [describe, html] = await Promise.all([
      describeImageCached(dataUrl, DEFAULTS.model, ''),
      restoreImageCached(dataUrl, DEFAULTS.model, ''),
    ])
    return [
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const model = typeof args.model === 'string' && args.model.trim() !== '' ? args.model.trim() : DEFAULTS.model
      let dataUrl
      let byteCount = 0
  ...
  ctx.logger.info(`dsh-sfversion 已就绪:visionTranslation + vision_glance + vision_restore_ui(模型 ${DEFAULTS.model})`)
}
