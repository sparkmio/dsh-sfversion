...
import { classifyVisionMode } from './vision-mode.js'
export { classifyVisionMode } from './vision-mode.js'
...
// DSH 当前的 prompt wire 只允许 text/image，不能把任意 Office/PDF 文件作为原生
// attachment 发出。这里使用不带 URI 的短文件标签：它是聊天中唯一可见的附件投影，
// 原始字节仍留在宿主内存；模型请求会通过 llm/stream 边界展开，不会把解析提示词写进历史。
function documentReferenceMarker(name, _token) {
  const label = String(name || 'document').replace(/[\[\]\r\n]/g, '_')
  return `[📎 ${label}]`
}

function parseDocumentReference(text) {
  ...
  // 兼容曾经生成过的无括号短标签，但只接受整行，避免把“📎 文件名 总结一下”
  // 的后半句误判为文件名。新版本统一生成上面的 [📎 文件名]。
  const plain = /(?:^|\n)📎\s+([^\n]+?)\s*(?=\n|$)/i.exec(value)
...
function stripDocumentTransfer(text) {
  ...
  if (reference) return [reference.before.trim(), `[已上传文档:${reference.name}]`, reference.after.trim()].filter(Boolean).join('\n')
...
export function apply(ctx) {
  ...
  let nextSignalId = 1
  // Short document references are deliberately host-memory-only. They are not
  // persisted to session history and expire like the browser-side references.
    ...
    void connection.rpc.handle('/sfv', async (endpoint, payload, signal) => {
      ...
      try {
        ...
        if (endpoint === 'attach') {
          ...
          return { ok: true, value: { marker: documentReferenceMarker(name, token), type: documentType(name, mime), bytes: bytes.byteLength } }
        }
  ...
  async function translateBlocks(blocks, signal, query) {
    ...
    for (const block of blocks || []) {
      ...
      } else if (block?.type === 'text' && typeof block.text === 'string' && (parseDocumentReference(block.text) || /\[\[SFV_DOCUMENT_V1\s/i.test(block.text))) {
  ...
  ctx.on('llm/stream', async (options, next) => {
    ...
    const translated = await Promise.all(messages.map(async (message) => ({ ...message, content: await translateDocumentBlocks(message.content, options?.signal, textQuery(message.content)) })))
    return next({ ...options, messages: translated })
  })
