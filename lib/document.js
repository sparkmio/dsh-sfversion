...
const PDF_RENDER_WORKER = fileURLToPath(new URL('./pdf-render-worker.js', import.meta.url))
const PDF_RENDER_TIMEOUT_MS = 90_000
const PDF_RENDER_MAX_OUTPUT_BYTES = 32 * 1024 * 1024

function pdfRenderExitDetail(code, signal, stderr) {
  const status = Number.isInteger(code) ? `退出码 0x${(code >>> 0).toString(16).toUpperCase().padStart(8, '0')}` : signal ? `被信号 ${signal} 终止` : '未返回退出码'
  const detail = String(stderr || '').replace(/\s+/g, ' ').trim()
  return `隔离 PDF 页面渲染器异常退出（${status}${detail ? `：${detail.slice(0, 240)}` : ''}）`
}

/**
 * Native canvas is intentionally run in a child process: a malformed PDF can
 * crash its native renderer with 0xC0000409, which cannot be caught in the
 * host process. A child failure becomes a normal parse fallback instead.
 */
function renderPdfPageToPng(sourceBytes, pageNo, signal) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const onAbort = () => {
      stop()
      finish(signal?.reason || new Error('操作已取消'))
    }
    try {
      child = spawn(process.execPath, [PDF_RENDER_WORKER], {
        stdio: ['pipe', 'pipe', 'pipe'],
        windowsHide: true,
        env: { ...process.env, ELECTRON_RUN_AS_NODE: '1' },
      })
    } catch (error) {
      finish(new Error(`无法启动隔离 PDF 页面渲染器：${error?.message || String(error)}`))
      return
    }
    timeout = setTimeout(() => {
      stop()
      finish(new Error(`隔离 PDF 页面渲染超时（${Math.floor(PDF_RENDER_TIMEOUT_MS / 1000)} 秒）`))
    }, PDF_RENDER_TIMEOUT_MS)
    signal?.addEventListener('abort', onAbort, { once: true })
    child.stdout.on('data', (chunk) => {
      const value = Buffer.from(chunk)
      outputBytes += value.byteLength
      if (outputBytes > PDF_RENDER_MAX_OUTPUT_BYTES) {
        stop()
        finish(new Error('隔离 PDF 页面渲染输出过大'))
        return
      }
      output.push(value)
    })
    child.stderr.on('data', (chunk) => { stderr = (stderr + Buffer.from(chunk).toString('utf8')).slice(-4096) })
    child.on('error', (error) => finish(new Error(`隔离 PDF 页面渲染器启动失败：${error?.message || String(error)}`)))
    child.on('close', (code, closeSignal) => {
      if (settled) return
      let result
      try { result = JSON.parse(Buffer.concat(output).toString('utf8')) } catch {
        finish(new Error(pdfRenderExitDetail(code, closeSignal, stderr)))
        return
      }
      if (code !== 0 || result?.ok !== true || typeof result.png !== 'string') {
        finish(new Error(result?.error || pdfRenderExitDetail(code, closeSignal, stderr)))
        return
      }
      try { finish(undefined, { bytes: new Uint8Array(Buffer.from(result.png, 'base64')) }) }
      catch (error) { finish(new Error(`隔离 PDF 页面渲染结果无效：${error?.message || String(error)}`)) }
    })
    try {
      child.stdin.end(JSON.stringify({ pdf: Buffer.from(sourceBytes).toString('base64'), page: pageNo }))
    } catch (error) {
      stop()
      finish(new Error(`无法传递 PDF 页面渲染请求：${error?.message || String(error)}`))
    }
  })
}
...
async function parsePdf(bytes, name, signal) {
  // PDF.js may transfer/detach the Uint8Array passed to getDocument(). Keep an
  // owned copy for fallback extraction and the isolated page renderer.
  const sourceBytes = bytes.slice()
  let pdf
  try {
    const pdfjs = await import('pdfjs-dist/legacy/build/pdf.mjs')
    const task = pdfjs.getDocument({ data: bytes.slice(), disableWorker: true, useWorkerFetch: false, isEvalSupported: false })
    ...
    for (let pageNo = 1; pageNo <= pdf.numPages; pageNo++) {
      ...
      if (pageTextCount === 0 && extractedImageCount === 0) {
        try {
          const rendered = await renderPdfPageToPng(sourceBytes, pageNo, signal)
    ...
    return fallbackPdf(sourceBytes, name, error?.message || String(error))
  } finally {
