...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function documentMarker(name, mime, bytes) {
    ...
    function UploadButton(props) {
      ...
      const onPick = async (evt) => {
        ...
        if (!isImage && !docMime) {
          st.update({ status: 'error', error: '仅支持 png/jpeg/webp/gif 图片或 docx/pptx/xlsx/pdf/md/markdown 文档' })
          return
        }
        ...
        try {
          ...
          if (!isImage) {
            const marker = documentMarker(name, docMime, bytes)
            const inputActions = props !== undefined && props !== null ? props.inputActions : undefined
            if (inputActions === undefined || inputActions === null || typeof inputActions.setDraft !== 'function') throw new Error('无法把文档加入输入框(文本草稿接口不可用)')
            const current = inputActions.state && typeof inputActions.state.get === 'function' ? inputActions.state.get()?.draft || '' : ''
      ...
      return React.createElement('div', { className: 'sfv-upload', title: '上传图片或文档:发送后自动分离文字、图片和位置上下文' },
        React.createElement('label', null,
          React.createElement('input', {
            type: 'file',
            accept: 'image/png,image/jpeg,image/webp,image/gif,.docx,.pptx,.xlsx,.pdf,.md,.markdown',
