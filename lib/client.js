...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 文档引用只存在客户端内存；发送时通过 Connection RPC 序列化，绝不进入 draft/会话历史。
      ...
      const onPick = async (evt) => {
        ...
        try {
          ...
          if (!isImage) {
            ...
            const snapshot = inputActions?.state?.getSnapshot?.() || inputActions?.state?.get?.()
            if (inputActions === undefined || inputActions === null || snapshot === undefined || snapshot === null
              || typeof snapshot.draft !== 'string' || typeof snapshot.draftRev !== 'number'
              || sessionCtx === undefined || sessionCtx === null || typeof sessionCtx.bail !== 'function') {
            ...
            documentRefs.set(ref, { name, mime: docMime, bytes })
            const reference = { source: DOCUMENT_SOURCE, ref, label: name, clipboardText: `[文档:${name}]` }
            const at = snapshot.draft.length
            const accepted = sessionCtx.bail(sessionCtx, 'slash/input-insert-reference', {
              reference,
    ...
    function registerDocumentReferenceSource(ctx) {
      ...
      const source = {
        ...
        onPick: () => undefined,
        codec: {
          clipboardText(ref) { return documentRefs.get(ref)?.name ? `[文档:${documentRefs.get(ref).name}]` : '[文档]' },
          async serialize(ref, signal) {
            ...
            const result = await connection.rpc.call('/sfv', 'inspect', {
              name: item.name,
              mime: item.mime,
              base64: bytesToBase64(item.bytes),
            }, signal)
            if (!result?.ok) throw new Error(result?.error?.message || '文档解析失败')
            const text = result.value?.text
            if (typeof text !== 'string' || text.trim() === '') throw new Error('文档未提取出可发送内容')
            return text
          },
        },
      }
