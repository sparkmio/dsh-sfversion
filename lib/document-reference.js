...
/**
 * Build an invisible, durable attachment descriptor. Metadata stays inside
 * the zero-width payload so the chat transcript can render a native-looking
 * file card after the composer draft has been cleared; it is never a visible
 * prompt fragment or URI.
 */
export function documentReferenceMarker(name, token, mime = '') {
  const value = String(token || '')
  if (value === '') throw new Error('文档附件 token 不能为空')
  const payload = JSON.stringify({ v: 2, t: value, n: String(name || 'document'), m: String(mime || '') })
  return ZERO_WIDTH_START + encodeZeroWidth(payload) + ZERO_WIDTH_END
}

function parseHiddenPayload(value) {
  const decoded = decodeZeroWidth(value)
  ...
  } catch { /* Pre-1.2.1 markers encode the token directly. */ }
  return { token: decoded, name: '', mime: '' }
}
...
export function parseDocumentReference(text) {
  ...
  if (hidden) {
    const reference = parseHiddenPayload(hidden[1])
  ...
  if (markdown) {
    ...
    return { name, mime: '', token, label: markdown[0], full: markdown[0], before: value.slice(0, markdown.index), after: value.slice(markdown.index + markdown[0].length) }
  }

  const short = /\[📎\s+([^\]\n]+)\]/i.exec(value)
  if (short) {
    const name = short[1].trim()
    return { name, mime: '', token: undefined, label: short[0], full: short[0], before: value.slice(0, short.index), after: value.slice(short.index + short[0].length) }
  }
  ...
  return { name, mime: '', token: undefined, label: full, full, before: value.slice(0, index), after: value.slice(index + full.length) }
}